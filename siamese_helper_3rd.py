...
def SNN_KNN():
    ...
    train_X = (np.load('Data\\Saideep\\3rd\\train_X_3rd.npy', allow_pickle=True).astype('float16') + 100.0) / 100.0
    train_Y = np.load('Data\\Saideep\\3rd\\train_Y_3rd.npy', allow_pickle=True).astype('int')
    test_X = (np.load('Data\\Saideep\\3rd\\test_X_3rd.npy', allow_pickle=True).astype('float16') + 100.0) / 100.0
    test_Y = np.load('Data\\Saideep\\3rd\\test_Y_3rd.npy', allow_pickle=True).astype('int')
    ...
    print('avg:')
    print(avg_err)
    np.save('results\\3rd_floor\\snn_knn_classification_3rd.npy', acc_tmp)
...
def SNN_KNN_aug():
    ...
    train_X = (np.load('Data\\Saideep\\3rd\\train_X_3rd.npy', allow_pickle=True).astype('float16') + 100.0) / 100.0
    train_Y = np.load('Data\\Saideep\\3rd\\train_Y_3rd.npy', allow_pickle=True).astype('int')
    test_X = (np.load('Data\\Saideep\\3rd\\test_aug_KNN_X_3rd.npy', allow_pickle=True).astype('float16') + 100.0) / 100.0
    test_Y = np.load('Data\\Saideep\\3rd\\test_Y_3rd.npy', allow_pickle=True).astype('int')
    ...
    for month in range(15):
        ...
        test_y_coord = uji.labels_to_coords(test_Y[month])
        # compute the distances
        dists = uji.compute_distances(pred_y_coord, test_y_coord).flatten()-1
        acc_tmp[month] = np.mean(dists)
        # store the distances as a 2D array
        accuracies[month + 1] = dists
        print(np.mean(dists))
    ...
    print('avg:')
    print(avg_err)
    np.save('results\\3rd_floor\\snn_knn_classification_3rd.npy', avg_err)
